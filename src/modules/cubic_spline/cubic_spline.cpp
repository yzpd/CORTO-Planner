...
{
    const int max_idx = spline_data_.n_points - 2;

    if(x >= spline_data_.x_data(spline_data_.n_points-1))
    {
        return max_idx;
    }
    if(x <= spline_data_.x_data(0))
    {
        return 0;
    }
    int idx;
    ...
    {
        idx = int(floor(x/spline_data_.delta_x));
    }
    else
    {
...
double CubicSpline::getPoint(double x) const
{
    ...
    // x = unwrapInput(x);
    // if (x == spline_data_.x_data(spline_data_.n_points-1)) {
    //     // bool is_same = spline_data_.y_data(spline_data_.n_points-1) == spline_params_.a(spline_data_.n_points-1);
    //     return spline_data_.y_data(spline_data_.n_points-1);
    // }
    // compute index
    index = getIndex(x);
...
double CubicSpline::getDerivative(double x) const
{
    ...
    // if (x == spline_data_.x_data(spline_data_.n_points-1)) {
    //     // bool is_same = spline_data_.y_data(spline_data_.n_points-1) == spline_params_.a(spline_data_.n_points-1);
    //     return spline_params_.b(spline_data_.n_points-2) + 2.0*spline_params_.c(spline_data_.n_points-2)*spline_data_.delta_x + 3.0*spline_params_.d(spline_data_.n_points-2)*spline_data_.delta_x*spline_data_.delta_x;
    // }
    index = getIndex(x);
    x_i = spline_data_.x_data(index);
...
double CubicSpline::getSecondDerivative(double x) const
{
    ...
    // if (x == spline_data_.x_data(spline_data_.n_points-1)) {
    //     // bool is_same = spline_data_.y_data(spline_data_.n_points-1) == spline_params_.a(spline_data_.n_points-1);
    //     return 2.0*spline_params_.c(spline_data_.n_points-2) + 6.0*spline_params_.d(spline_data_.n_points-2)*spline_data_.delta_x;
    // }
    index = getIndex(x);
    x_i = spline_data_.x_data(index);
